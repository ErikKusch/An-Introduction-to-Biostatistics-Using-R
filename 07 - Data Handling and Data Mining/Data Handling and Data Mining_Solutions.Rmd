...
options(java.parameters = "-Xmx8g") # allocate more RAM to your java processes called by R
```
...
package_vec <- c("xlsx", # we need this package for loading excel files
...
`R` offers a plethora of approaches to data loading and you will usually be taught the `read.table()` command in basic biostatistics courses. However, I have found to prefer the functionality provided by the `xlsx` package since most data recording is taking place in Excel. Therefore, we will use this for data loading for now.

```{r DataLoading}
Data_df_base <- read.xlsx(file = paste(Dir.Data, "/SparrowData.xls", sep=""), header = TRUE, sheetIndex = "Data (containing Errors)")
...
The weight data contained within our data frame should be numeric and this pose no issue to the `summary()` function. However, as shown in the next section, it is currently of type `r class(Data_df$Weight)` which leads the `summary()` function to work improperly.
```{r SummaryWE}
...
Making data inspection more easy, one may which to automate the use of the `summary()` function. However, this only makes sense, when every data column is presenting data in the correct class type. Therefore, we will first fix the column classes and then automate the use of the `summary()` command.
...
summary(Data_df$Weight)
```
We still have at least one data value exceeding the biological feasible range of weights. However, this is an issue of column content and not column type and so we will deal with this later.  

<!-- Below you can see a representation of the above transformation approaches to assess the performance of the procedures. -->
<!-- ```{r plottingpanesWeight} -->
<!-- par(mfrow=c(2,2)) # plotting panes as 2 by 2 -->
<!-- hist(as.numeric(Data_df_base$Weight), main = "Numeric(Data)", breaks = 100) -->
<!-- hist(as.numeric(as.character(Data_df_base$Weight)), main = "Numeric(Character(Data))", breaks = 100) -->
<!-- hist(as.numeric(gsub(pattern = ",", replacement = ".", x = Data_df_base$Weight)), main = "Numeric(CommaFix(Data))", breaks = 100) -->
<!-- ``` -->
...
write.xlsx(Data_df, file = paste(Dir.Data, "/SparrowData.xls", sep=""),
           sheetName = "Data (Fixed)", append = TRUE)
