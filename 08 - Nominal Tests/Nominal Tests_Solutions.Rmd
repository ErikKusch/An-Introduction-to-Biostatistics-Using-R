...
\begin{center}
\textit{Are the sexes represented in equal proportions?}
\end{center}
...
\begin{center}
\textit{Are the sites dominated by predators?}
\end{center}
...
The **null hypothesis** reads: **Class assignment probablities do not change within different treatments**.  

## Preparing The Data

\begin{center}
\textit{Do sex ratios change over time?}
\end{center}
...
# preparing plotting
probs <- cbind(
  prop.table(mcnemar_matrix_change[1,]),
  prop.table(mcnemar_matrix_change[2,])
)
colnames(probs) <- c("Now", "Next Year")
# plotting
barplot(probs, main="Abundances of the sexes among study organisms",
        xlab="Years", ylab = "Proportions", col=c("Red", "Blue"))
```
...
\begin{center}
\textit{Are colours related to sex or predator parameters?}
\end{center}
...
colnames(probs) <- c("No", "Yes")

barplot(probs, main="Colour Variations of the common House Sparrow",
        xlab="Predator Presence", ylab = "Proportion", col=c("Black", "saddlebrown", "Grey"))
```
...
# Predator Presence
CochPredator.Presence <- Data_df_base$Predator.Presence

# Sex
CochSex <- Data_df_base$Sex
...
```
Based on this result, we **reject the null hypothesis** of opredator presence records being equally likely to occur.  

*Lastly*, recall the binomial test run on the **sex** data records which exhibit an almost even 50/50 split.  

Whilst none of these test give us any idea about the overlap of similar assignments along these variable vectors, a 50/50 split (sex) can never link up comparably with a roughly 30/70 split (predator presence and binary colour). Therefore, wo could hypothesize a linkage of predator presence and colour rather than sex and colour morphs.

# Chi-Squared
The Chi-Squared (also known as Chi\textsuperscript{2}) Test can be regarded as a functional extension of the binomial test and is used to test the similarity of class assignment proportions for a categorical/nominal variable. Unlike the binomial test, however, this test is not constrained to binomial records alone.  

The null hypothesis states that: **Every class assignment conatined within a given variable is equally likely**.  
...
The two sample Chi-Squared approach let's us identify whether class assignment proportions of one variable differ when they are considered in a dependency of another nominal variable.  

### Sexual Dimorphism

\begin{center}
\textit{Are colours of Passer domesticus related to their sexes?}
\end{center}
...
\begin{center}
\textit{Are colours of Passer domesticus related to predator parameters?}
\end{center}
...
```
Nope, no link here. We have to **accept the null hypotesis** and conclude that there may be no causal link of predator type and sparrow colour.  

\hfill \linebreak
\begin{center}
\textit{Are nesting sites of Passer domesticus related to predator parameters?}
\end{center}
