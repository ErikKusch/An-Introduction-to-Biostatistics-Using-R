...
\begin{table}[h!]
  \begin{tabular}{|p{1.5cm}p{1.8cm}p{5cm}p{2cm}|} \hline
    ...
    02/04/2019    & 15:00 - 16:30 & (1) An Introduction to Basic Statistics for Biologists                                 & CIP-POOL \\\hline
    09/04/2019      & 15:00 - 16:30 & (2) Introduction to R                                               & CIP-POOL \\\hline
    \multicolumn{4}{|l|}{\textbf{II.) Basic statistical terminology}} \\
      09/04/2019      & 15:00 - 16:30 & (3) A Primer for Statistical Tests                                  & CIP-POOL \\\hline
      14/04/2019      & 15:00 - 16:30 & (4) Descriptive Statistics                                          & CIP-POOL \\\hline
      23/04/2019      & 15:00 - 16:30 & (5) Data Visualisation                                              & CIP-POOL \\\hline
      30/04/2019      & 15:00 - 16:30 & (6) Inferential Statistics, Hypotheses and our Research Project                        & CIP-POOL \\\hline
  \end{tabular}
\end{table}
\end{footnotesize}
...
\begin{table}[h!]
  \begin{tabular}{|p{1.5cm}p{1.8cm}p{5cm}p{2cm}|} \hline
    ...
    \multicolumn{4}{|l|}{\textbf{III.) Handling Data}} \\
      07/05/2019      & 15:00 - 16:30 & (7) Data Handling and Data Mining                                   & CIP-POOL \\\hline
    \multicolumn{4}{|l|}{\textbf{IV.) Non-parametric tests}} \\
      14/05/2019      & 15:00 - 16:30 & (8) Nominal Tests                                                   & CIP-POOL \\\hline
      21/05/2019      & 15:00 - 16:30 & (9) Correlation Tests                                               & CIP-POOL \\\hline
      28/05/2019      & 15:00 - 16:30 & (10) Ordinal and Metric Tests for two-sample situations              & CIP-POOL \\\hline
      04/06/2019      & 15:00 - 16:30 & (11) Ordinal and Metric Tests for more than two-sample situations   & CIP-POOL \\\hline
    \multicolumn{4}{|l|}{\textbf{V.) Parametric tests}} \\
      02/07/2019     & 15:00 - 16:30 & (12) Simple Parametric Tests                                        & CIP-POOL \\\hline
    \multicolumn{4}{|l|}{\textbf{VI.) Closing}} \\
      Handout      &  & (13) Summary and an Outlook on Advanced Statistics                                 &  \\\hline
  \end{tabular}
\end{table}
\end{footnotesize}
...
\begin{columns}[T]
  ...
  We will:
    ...
    \pause
    \item Fully reproducible analyses (located here: \url{https://github.com/ErikKusch/An-Introduction-to-Biostatistics-Using-R})
    \end{itemize}
  \end{column}
  \begin{column}{.5\linewidth}
...
**\uline{B.Sc.}** at **Technische Universität Dresden**:  
\begin{itemize}
  \item \textbf{Course} "Biostatistik für Biologen" by \textit{Dr. Matthias Rudolf}
\end{itemize}
  \visible<2->{
    $\rightarrow$ \textbf{first introduction to biostatistics}
  }
\begin{columns}[T]
  \begin{column}{.5\linewidth}
    \visible<3->{
    \begin{itemize}
      \item \textbf{B.Sc. thesis} on landscape-scale vegetation dynamics based on remote sensing data with: 
        \begin{itemize}
        \item \textit{Dr. Alistair Seddon} (Universitetet i Bergen)
        \item \textit{Prof. Dr. Klaus Reinhardt} (Technische Universität Dresden)
        \end{itemize}
    \end{itemize}
    $\rightarrow$ \textbf{first steps in R!}
    }
  \end{column}
  \begin{column}{.5\linewidth}
  \visible<4->{
    \centering
    \includegraphics[width=.9\linewidth]{Figures/ROC_Full_Numbers.jpg}
    \small
    \vspace{-1cm}
      \begin{table}[c]
        \centering
          ...
          \begin{tabular}{r|ccccc}
            & \multicolumn{5}{c}{\textit{Present State}} \\
           \hline
           \textit{Past State} & Boreal Forest     & Dwarf Shrub     & Shrub     & Barren/Moss     & Tundra/Sedge     \\
            Boreal Forest     & {\color{green}{93.39\%}} & 0.78\%  & 4.56\%  & 0.06\%  & 1.21\% \\
            Dwarf Shrub     & 5.53\%  & 53.5\%  & {\color{orange}{29.55\%}} & 0.02\%  & 11.4\% \\
            Shrub     & 12.12\% & 17.85\% & 68.51\% & 0\%     & 1.53\% \\
            Barren/Moss     & 1.23\%  & 0.19\%  & 0\%     & {\color{green}{76.8\%}} & {\color{orange}{21.77\%}} \\
            Tundra/Sedge     & 5.3\%   & {\color{orange}{26.18\%}} & 4.1\%   & 2.52\%  & 61.89\% \\
          \hline
          \end{tabular}}
        \label{tab:Contingency_AK}
    \end{table}
  }
  \end{column}
\end{columns}
...
### Statistics (**M.Sc.**)
**\uline{M.Sc.}** at **Universitetet i Bergen**:
\begin{itemize}
\visible<1->{
  \item Bayesian \textbf{Summer School} by \textit{Dr. Joseph Chipperfield \& Partners}
  \item \textbf{Course} "Statistical Learning" by \textit{Dr. Yushu Li}
  \item \textbf{Course} "Ordination and Gradient Analysis" by \textit{Dr. Richard Telford}
  \item \textbf{Course} "Biological Data Analysis II" by \textit{Dr. Richard Telford}
  \item \textbf{Course} "Biostatistics" by \textit{Dr. Knut Helge Jensen} \linebreak
}\visible<2->{
  \item \textbf{M.Sc. thesis} on vegetation memory effects on landscape-scale and the link with plant functional traits with: 
        \begin{itemize}
        \item \textit{Dr. Alistair Seddon} (Universitetet i Bergen)
        \item \textit{Dr. Richard Davy} (Nansen Environmental and Remote Sensing Center)
        \end{itemize}
}
\end{itemize}
\visible<3>{
\centering $\rightarrow$ \textbf{Developing my grasp on statistical theory and practice in R}
}

### Statistics (**Research Assistant**)
**\uline{Research Assistant}** at **University of Kyoto**:  
\begin{itemize}
  \item Analysing behavioural interactions of Japanese macaques using complex social networks  
\end{itemize}

**\uline{Research Assistant}** at **Universität Leipzig**:  
\begin{itemize}
  \item Generating data with various implemented odour effects on which to test the methods  
  \item Analysing primate odour profiles  
  \item Assessing a variety of statistical methods for analytical power in identifying odour effects
  \item Teaching the basics of biostatistics (this is where you come in)
\end{itemize}


\visible<2>{
\centering $\rightarrow$ \textbf{Putting experience in statistics and R to the test}
}
