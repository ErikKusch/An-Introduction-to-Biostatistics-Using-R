...
```{r Spearman}
set.seed(42)
cor.test(x = c(1:1000), y = sample(c(1:1000), size = 1000, replace = FALSE), method = "spearman")
```
\normalsize
...
\begincols[T]
  ...
  \pause
    ...
    \begin{itemize}
    \item \textit{Competition:} Are home ranges related to their stations?
  ...
  \pause
    \textbf{Kendall's Tau}
    \begin{itemize}
    \item \textit{Climate Warming/Extremes:} Do heavier sparrows have heavier/less eggs? 
    \item \textit{Competition:} Do heavier sparrows flock together?
    \end{itemize} 
\endcol
\begincol{.5\linewidth}
  \pause
    \textbf{Spearman \& Pearson}
    \begin{itemize}
    ...
    \item \textit{Competition:} Do population/flock size correlate with latitude?
    \end{itemize} 
\endcol
\endcols
